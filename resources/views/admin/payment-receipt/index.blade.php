              ...
              <div class="card pt-2">
                ...
                <div class="card-body">
                    <div class="row align-items-center mb-4 cart_filter_box pb-3">
                        <div class="col">
                            <h4>@lang('quickadmin.payment-receipt-management.title')</h4>
                        </div>

                        <div class="col-md-auto col-12 mt-md-0 mt-3">
                            <div class="supplier-btn-area">
                                <div class="">
                                    @can('payment_receipt_create')
                                    <button type="button" class="addnew-btn addRecordBtn sm_btn circlebtn"  data-href="{{ route('payment-receipt.create')}}" title="@lang('quickadmin.qa_add_new')"><x-svg-icon icon="add" /></button>
                                    @endcan
                                </div>
                                <div class="">
                                    @can('payment_receipt_print')
                                    <a href="{{ route('payment-receipt.print') }}" class="btn printbtn h-10 col circlebtn"  id="print-button" title="@lang('quickadmin.qa_print')"> <x-svg-icon icon="print" /></a>
                                    @endcan
                                </div>
                                {{-- <div class="col-auto pl-1">
                                    @can('payment_receipt_export')
                                    <a href="{{ route('payment-receipt.export') }}" class="btn excelbtn h-10 col circlebtn"  id="excel-button" title="@lang('quickadmin.qa_excel')"><x-svg-icon icon="excel" /></a>
                                    @endcan
                                </div> --}}
                            </div>
                        </div>
...
$(document).ready(function () {
    ...
    $("body").on("click", ".edit-payment-receipt-btn", function () {
        var hrefUrl = $(this).attr('data-href');
        $.ajax({
            type: 'get',
            url: hrefUrl,
    ...
    /// Add Payment Receipt
    $(document).on('submit', '#AddPaymentReceiptForm', function (e) {
        e.preventDefault();

        $("#AddPaymentReceiptForm button[type=submit]").prop('disabled',true);
        $(".error").remove();
        ...
        $.ajax({
            ...
            success: function (response) {
                    ...
                    var title = "{{ trans('quickadmin.payment_receipts.payment_receipt') }}";
                    showToaster(title,alertType,message);
                    $('#centerModal #AddPaymentReceiptForm')[0].reset();
                    DataaTable.ajax.reload();
                    $("#centerModal #AddPaymentReceiptForm button[type=submit]").prop('disabled',false);
            },
            error: function (xhr) {
    ...
    $(document).on('submit', '#EditPaymentReceiptForm', function (e) {
        e.preventDefault();
        $("#EditPaymentReceiptForm button[type=submit]").prop('disabled',true);
        $(".error").remove();
        $(".is-invalid").removeClass('is-invalid');
        ...
        $.ajax({
            ...
            success: function (response) {
                    ...
                    var title = "{{ trans('quickadmin.payment_receipts.payment_receipt') }}";
                    showToaster(title,alertType,message);
                    $('#EditPaymentReceiptForm')[0].reset();
                    DataaTable.ajax.reload();
                    $("#EditPaymentReceiptForm button[type=submit]").prop('disabled',false);
            },
            error: function (xhr) {
                var errors= xhr.responseJSON.errors;
                for (const elementId in errors) {
                    $("#EditPaymentReceiptForm #"+elementId).addClass('is-invalid');
                    var errorHtml = '<div><span class="error text-danger">'+errors[elementId]+'</span></';
                    $(errorHtml).insertAfter($("#EditPaymentReceiptForm #"+elementId).parent());
                }
                $("#EditPaymentReceiptForm button[type=submit]").prop('disabled',false);
            }
        });
    ...
    $(document).on('submit', '.deleteForm', function(e) {
        ...
        if (willDelete) {
            ...
            error: function (xhr) {
                ...
                swal("{{ trans('quickadmin.payment_receipts.payment_receipt') }}", 'Something Went Wrong!', 'error');
            }
            });
        }
        });
    ...
    $(document).on('submit', '.suppliermodalbody #centerModal #AddForm', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                    ...
                    $('#AddPaymentReceiptForm #supplier_id').append(newOption2).trigger('change');
